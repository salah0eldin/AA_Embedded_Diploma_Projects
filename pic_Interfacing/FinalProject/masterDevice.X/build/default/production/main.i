...
volatile uint8_t timerTicks = 1;
volatile uint8_t timeDate[7] = {0};
volatile uint8_t temperature = 0;
uint8_t temperatureAddress = 0x00;
uint8_t temperatureState = temp_state_idle;
uint8_t previousTemperatureState = temp_state_idle;
uint8_t externalEEPROMAddress = 0x00;
...
int main(void) {
    ...
    I2C1_WriteRead(0x50, &externalEEPROMAddress, 1, &externalEEPROMAddress, 1);
    while (I2C1_IsBusy());

    if (externalEEPROMAddress == 0xFF) {
        externalEEPROMAddress = 0x08;
    }
    ...
    while (1) {
        ...
        I2C1_WriteRead(0x4D, &temperatureAddress, 1, &temperature, 1);
        while (I2C1_IsBusy());
        ...
        if (temperatureState != previousTemperatureState) {
            I2C1_Write(0x8, &temperatureState, 1);
            while (I2C1_IsBusy());
            ...
            if (temperatureState == temp_state_max) {
                ...
                timeDate[3] = timeDate[2];
                timeDate[2] = timeDate[1];
                timeDate[1] = timeDate[0];
                timeDate[0] = externalEEPROMAddress;
                externalEEPROMAddress += 8;

                I2C1_Write(0x50, &timeDate, 7);
                while (I2C1_IsBusy());
                _delay((unsigned long)((10)*(8000000/4000.0)));


                timeDate[0] = 0x00;
                timeDate[1] = externalEEPROMAddress;
                I2C1_Write(0x50, &timeDate, 2);
                while (I2C1_IsBusy());
                _delay((unsigned long)((10)*(8000000/4000.0)));
            } else {
