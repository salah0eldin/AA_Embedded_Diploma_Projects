...
# 11 "MCAL_Layer/Timers/timer3.c"
# 1 "MCAL_Layer/Timers/timer3.h" 1
# 14 "MCAL_Layer/Timers/timer3.h"
# 1 "MCAL_Layer/Timers/../GPIO/gpio.h" 1
# 21 "MCAL_Layer/Timers/../GPIO/gpio.h"
...
# 23 "MCAL_Layer/Timers/../GPIO/gpio.h" 2
# 53 "MCAL_Layer/Timers/../GPIO/gpio.h"
typedef enum
{
    GPIO_LOW = 0,
    GPIO_HIGH
...
# 16 "MCAL_Layer/Timers/timer3.h" 2
# 59 "MCAL_Layer/Timers/timer3.h"
typedef enum
{
...
typedef struct
{
    ...
    void (* TIMR3_InterruptHandler)(void);




    timer3_prescaler_t prescaler_val;
...
Std_ReturnType Timer3_Init(const timer3_t *timer)
{
    ...
    else
    {
        ...
        (PIE2bits.TMR3IE = 1);
        (PIR2bits.TMR3IF = 0);
        TIMR3_InterruptHandler = timer->TIMR3_InterruptHandler;
# 80 "MCAL_Layer/Timers/timer3.c"
        (INTCONbits.GIE = 1);
        (INTCONbits.PEIE = 1);
...
# 127 "MCAL_Layer/Timers/timer3.c"
Std_ReturnType Timer3_Write_Value(const timer3_t *timer, uint16 val)
{
    Std_ReturnType ret = 0x01;
...
# 152 "MCAL_Layer/Timers/timer3.c"
Std_ReturnType Timer3_Read(const timer3_t *timer, uint16 *val)
{
    Std_ReturnType ret = 0x01;
    uint8 l_tmr3l = 0, l_tmr3h = 0;

    if (((void*)0) == timer || ((void*)0) == val)
    {
        ret = 0x00;
    }
    else
...
# 178 "MCAL_Layer/Timers/timer3.c"
static __attribute__((inline)) void Timer3_Mode_Select(const timer3_t *timer)
{
    ...
    {
        (T3CONbits.TMR3CS = 1);
        if(0 == timer->timer3_counter_sync)
        {
            (T3CONbits.T3SYNC = 0);
        }
        else if(1 == timer->timer3_counter_sync)
        {
            (T3CONbits.T3SYNC = 1);
        }else{ }
    }
...
static __attribute__((inline)) void Timer3_RW_Mode_Select(const timer3_t *timer)
{
    if(1 == timer->timer3_rw_mode)
    {
        (T3CONbits.RD16 = 1);
    }
    else if(0 == timer->timer3_rw_mode)
    {
        (T3CONbits.RD16 = 0);
    }
    else{ }
}
# 224 "MCAL_Layer/Timers/timer3.c"
void TIMR3_ISR(void)
{
    ...
    if(TIMR3_InterruptHandler)
    {
        TIMR3_InterruptHandler();
    }else{ }
