...
# 15 "MCAL_Layer/GPIO/../std_types.h" 2
# 44 "MCAL_Layer/GPIO/../std_types.h"
...
# 23 "MCAL_Layer/GPIO/gpio.h" 2
# 53 "MCAL_Layer/GPIO/gpio.h"
typedef enum
{
    GPIO_LOW = 0,
    GPIO_HIGH
...
# 47 "MCAL_Layer/GPIO/gpio.c"
Std_ReturnType gpio_pin_set_direction(const pin_config_t *pin)
{
    Std_ReturnType ret = 0x01;

    if(((void*)0) == pin || pin->pin_num > 8 - 1)
    {
        ret = 0x00;
    }
    else
    {
        ...
        case GPIO_DIRECTION_INPUT:
            (*tris_registers[pin->port] |= ((uint8)1 << pin->pin_num));
            break;
...
# 86 "MCAL_Layer/GPIO/gpio.c"
Std_ReturnType gpio_pin_get_direction(const pin_config_t *pin, direction_t *direction_status)
{
    Std_ReturnType ret = 0x01;

    if(((void*)0) == pin || ((void*)0) == direction_status || pin->pin_num > 8 - 1)
    {
        ret = 0x00;
    }
    else
    {
        *direction_status = ((*tris_registers[pin->port] & ((uint8)1 << pin->pin_num)) >> pin->pin_num);
    }
    return ret;
}
# 114 "MCAL_Layer/GPIO/gpio.c"
Std_ReturnType gpio_pin_write(const pin_config_t *pin, logic_t logic)
{
    Std_ReturnType ret = 0x01;
    ...
    else
    {
        ...
        {
        case GPIO_LOW:
            (*lat_registers[pin->port] &= (~((uint8)1 << pin->pin_num)));
            break;
        case GPIO_HIGH:
            (*lat_registers[pin->port] &= (~((uint8)1 << pin->pin_num)));
            (*lat_registers[pin->port] |= ((uint8)1 << pin->pin_num));
...
Std_ReturnType gpio_pin_read(const pin_config_t *pin, logic_t *logic)
{
    ...
    if(((void*)0) == pin || ((void*)0) == logic || pin->pin_num > 8 - 1)
    {
        ret = 0x00;
    }
    else
    {
        *logic = ((*port_registers[pin->port] & ((uint8)1 << pin->pin_num)) >> pin->pin_num);
    }

    return ret;
}
# 181 "MCAL_Layer/GPIO/gpio.c"
Std_ReturnType gpio_pin_toggle(const pin_config_t *pin)
{
    Std_ReturnType ret = 0x01;
