...
#endif
#endif
    timer1_prescaler_t prescaler;    // @ref timer1_prescaler_t.
    uint16 preload;               // Value to write as start in TMR1L and TMR1H registers.
    uint8 mode : 1;               // Timer1 mode selection.
    uint8 counter_sync : 1;       // Timer1 External Clock Input Synchronization.
    uint8 osc_enable : 1;         // Timer1 Oscillator (Enable or Disable) for the system.
    uint8 rw_mode : 1;            // Timer1 Read/Write in one 16-bit or two 8-bit operation Mode.
    uint8 reserved : 4;
}timer1_t;
