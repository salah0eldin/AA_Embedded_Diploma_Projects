 ...
 */
static inline void ADC_Select_Volt_Ref(const adc_config_t *adc) {
    if (ADC_VOLT_REF_ENABLE == adc->volt_P_reference) {
        ADC_VOLTAGE_REF_P_ENABLE();
    } else {
        ADC_VOLTAGE_REF_P_DISABLE();
    }
    if (ADC_VOLT_REF_ENABLE == adc->volt_N_reference) {
        ADC_VOLTAGE_REF_N_ENABLE();
    } else {
        ADC_VOLTAGE_REF_N_DISABLE();
    }
}
